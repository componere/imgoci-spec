...
- Run the focused local gate with `mise exec -- moon run root:cue --summary minimal`. CI runs the same task through `moon ci --force --summary minimal`; `scripts/check_cue.sh` owns formatting, module consistency, vetting, and generated-schema drift, then invokes `conformance/check.sh` across every explicit pass/fail fixture.
- `conformance/v1/pass` and `conformance/v1/fail` contain complete release indexes whose directory declares the expected `#ReleaseIndex` result. The harness validates parsed values only; exact JSON bytes, selection, retrieval, referenced objects, and decoded content remain separate conformance concerns.
